...
void main() {
    ...
    // Hash Functions for GPU Rendering paper
    ...
    // check the roughness for a given fragment is above a certain threshold
    if ( roughness <= 0.3 ) {
        ivec2 scaled_xy = ivec2(xy * resolution_scale);
        ...
        if ( payload.geometry_id != -1 ) {
            ...
            // use importance sampling to select a light for final color computation
            for ( uint l = 0; l < active_lights; ++l ) {
                ...
                // compute the angle between the light and the normal of the triangle that has been hit:
                vec3 p_to_light = light.world_position - p_world.xyz;

                float point_light_angle = dot( normalize( p_to_light ), triangle_normal );

                // compute the distance between the light and the fragment position in the world space:
                float distance_sq = dot( p_to_light, p_to_light );
                float r_sq = light.radius * light.radius;

                // use these two values to determine whether this light should be considered for this fragment:
            ...
            // Once the accumulated value is greater than random value, have found the light to use
...
// use the motion vectors value to determine whether can combine the data
// for the current frame with the previous frame
...
void main() {
    ...
    // compute the moments for the current frame
