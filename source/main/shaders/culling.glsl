...
bool project_sphere(vec3 C, float r, float znear, float P00, float P11, out vec4 aabb) {
	// whether the sphere is fully behind the near plane.
	if (-C.z - r < znear)
		return false;

	// compute the minimum and maximum points on the x axis
	...
	// value in perspective-projected space
	...
	if (mesh_instance_index < count) {
	    ...
	    if ( frustum_visible ) {
	    	...
	    	if ( project_sphere(view_bounding_center.xyz, radius, z_near, projection_00, projection_11, aabb ) ) {
				...
				// reduce the bounding box to an individual pixel lookup
